...
const resolveEnharmonicName = (name: string, globalKey: string) => {
...
const smoothTimelineData = (rawTimeline: any[], totalDuration: number, minDuration = 3.0) => {
    ...
    let segments = rawTimeline.map((entry, i) => {
        ...
        return {
            ...
            conf: entry.main.conf,
            alts: entry.alts || [], // Preserve alts
            startTime: entry.time,
            duration: nextTime - entry.time
        };
    });
    let merged = [];
    segments.forEach(seg => {
        ...
        if (last && last.name === seg.name) {
            ...
            // Keep alts from the segment with higher confidence or just the first one
            if (seg.conf > last.conf) {
                last.alts = seg.alts;
            }
        } else {
            merged.push({ ...seg });
    ...
    for (let pass = 0; pass < 2; pass++) {
        ...
        for (let i = 0; i < merged.length; i++) {
            ...
            if (curr.duration < minDuration && prev) {
                prev.duration += curr.duration;
                // Don't update alts during denoise merging for stability
            }
            else if (prev && prev.name === curr.name) {
                prev.duration += curr.duration;
            }
...
const KeyTimeline: React.FC<KeyTimelineProps> = ({ timeline, duration, currentTime, globalKey }) => {
            ...
            {smoothedData.map((seg, i) => {
                ...
                return (
                    <div key={i} className="h-full transition-all hover:brightness-110 relative group" style={{ width: `${widthPct}%`, backgroundColor: getKeyColor(seg.name), opacity: 0.5 + (seg.conf * 0.5) }}>
                        <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 hidden group-hover:block bg-black/80 text-white text-[9px] px-2 py-1 rounded whitespace-nowrap pointer-events-none z-50 font-sans">{displayName} ({seg.duration.toFixed(1)}s)</div>
                    </div>
                );
            })}
...
const AnalysisHUD: React.FC<AnalysisHUDProps> = ({ isLight }) => {
    ...
    const smoothedKeys = useMemo(() => {
        ...
        return smoothTimelineData(timeline, midiData.duration, 2.0);
    }, [analysisData, midiData]);

    const keyInfo = useMemo(() => {
        const globalKey = analysisData?.music_theory?.key || "---";
        const defaultResult = { name: globalKey, conf: 90, alts: [] };

        if (!smoothedKeys || smoothedKeys.length === 0) return defaultResult;

        // Find the smoothed segment that covers the current time
        const currentEntry = smoothedKeys.find(seg => currentTime >= seg.startTime && currentTime < (seg.startTime + seg.duration));

        if (!currentEntry) return defaultResult;

        const resolvedName = resolveEnharmonicName(currentEntry.name, globalKey);

        const resolvedAlts = (currentEntry.alts || []).map(a => ({
            ...a,
            name: resolveEnharmonicName(a.name, globalKey)
        }));

        return {
            name: resolvedName,
            conf: Math.round(currentEntry.conf * 100),
            alts: resolvedAlts
        };
    }, [analysisData, currentTime, midiData, smoothedKeys]);
    ...
    useEffect(() => {
        ...
        if (!areNotesEqual(prevNotesRef.current, currentNotes)) {
            ...
            // Keep octave numbers for better clarity (e.g., C3, F4 instead of C, F)
            const noteNames = [...new Set(currentNotes.map(n => n.name))].sort();
    ...
    const dynamicStats = useMemo(() => {
        const findEvent = (map: any[], time: number) => {
        ...
        <div className={`fixed bottom-8 right-8 flex flex-col items-end space-y-4 z-40 select-none ${textColor}`}>
            ...
            <div className="flex flex-col items-end space-y-2 mb-4 pointer-events-none">
                <div className="flex items-start gap-3">
                    <div className="text-right flex flex-col items-end">
                        ...
                        <div className="text-3xl font-sans tabular-nums font-light transition-all duration-100 flex items-center justify-end gap-2 leading-none mt-1">
                        ...
                        {/* Aliases / Alternatives List - Always visible, horizontal layout with fixed height */}
                        <div className={`flex flex-row items-center justify-end gap-1.5 mt-2 h-[16px] overflow-hidden animate-in slide-in-from-top-2 fade-in duration-300`}>
                            <span className={`text-[8px] uppercase tracking-widest opacity-50 whitespace-nowrap`}>
                            ...
                            {chordData.aliases && chordData.aliases.length > 1 ? (
                                chordData.aliases.slice(1, 4).map((alias, idx) => (
                                    <span key={idx} className="flex items-center gap-1.5 whitespace-nowrap">
                                        ...
                                        <span className={`text-[10px] font-sans tabular-nums opacity-60 border-b border-dashed border-white/10 truncate max-w-[80px]`}>
                                            {alias}
                                        </span>
                                    </span>
                                ))
                            ) : (
                                <span className={`text-[10px] font-sans tabular-nums opacity-30`}>---</span>
                            )}
                        </div>
                    </div>
                ...
                <div className="flex items-center gap-3 mt-2">
                    <div className="text-right">
                        ...
                        <div className="h-6 flex items-center justify-end gap-1 flex-wrap max-w-[200px]">
                            {activeNoteNames.length > 0 ? (activeNoteNames.map((note, i) => (<span key={i} className={`px-1.5 py-0.5 rounded text-xs font-sans tabular-nums border shadow-sm ${noteBg}`}>{note}</span>))) : <span className={`text-xs font-sans tabular-nums ${textDim}`}>...</span>}
                        </div>
                    </div>
            ...
            <div className={`flex flex-col items-end space-y-2 border rounded-xl p-4 transition-all duration-500 ${panelBg} ${borderCol} pointer-events-auto`}>
                <div className="flex items-center justify-between w-full gap-8">
                    ...
                    {/* FPS & Status */}
                    <div className={`flex items-center gap-3 text-[9px] font-sans tabular-nums ${textDim}`}>
                ...
                {isAnalyzing ? (
                    ...
                    <div className="flex items-center gap-6 opacity-90 animate-in fade-in slide-in-from-right-4 duration-500">
                        <div className="flex flex-col items-end"><span className={`text-[9px] ${textDim}`}>BPM</span><span className="font-light font-sans tabular-nums tabular-nums">{dynamicStats.bpm}</span></div>
                        <div className={`w-px h-6 ${borderCol}`}></div>
                        <div className="flex flex-col items-end"><span className={`text-[9px] ${textDim}`}>METER</span><span className="font-light font-sans tabular-nums tabular-nums text-midi-accent">{dynamicStats.meter}</span></div>
                        <div className={`w-px h-6 ${borderCol}`}></div>
                        <div className="flex flex-col items-end">
                            <div className="flex items-center gap-2">
                                <span className={`text-[9px] ${textDim}`}>KEY</span>
                                <span className={`text-[9px] font-sans tabular-nums ${textDim} w-10 text-right`}>{keyInfo.conf}%</span>
                            ...
                            <span className="font-sans font-extralight text-2xl leading-none mt-1 tracking-tight">{keyInfo.name}</span>
                            {/* Always show alternative key section with fixed height */}
                            <div className={`text-[9px] font-sans tabular-nums mt-0.5 inline-flex items-center gap-1 min-w-[120px] h-[14px] ${textDim}`}>
                                <span className="opacity-50">or</span>
                                {keyInfo.alts.length > 0 && keyInfo.alts[0] ? (
                                    <>
                                        <span className="font-bold border-b border-dashed border-current">{keyInfo.alts[0].name}</span>
                                        <span className="opacity-70 tabular-nums">({Math.round(keyInfo.alts[0].conf * 100)}%)</span>
                                    </>
                                ) : (
                ...
                {isExpanded && !isAnalyzing && analysisData?.music_theory?.key_timeline && (
                    <div className="w-full pt-2 animate-in slide-in-from-top-2 fade-in duration-300 border-t border-black/5 mt-2">
                        <div className="flex justify-between text-[9px] uppercase tracking-widest opacity-50 mb-1 font-sans tabular-nums">
                        ...
                        <div className="flex gap-2 mt-2 justify-end">
                            <div className="flex items-center gap-1 text-[9px] opacity-50 font-sans tabular-nums"><div className="w-2 h-2 rounded-full bg-red-400"></div> {t('analysis.major', { defaultValue: 'Major' })}</div>
                            <div className="flex items-center gap-1 text-[9px] opacity-50 font-sans tabular-nums"><div className="w-2 h-2 rounded-full bg-blue-400"></div> {t('analysis.minor', { defaultValue: 'Minor' })}</div>
                        </div>
                    </div>
                )}
            </div>
